...
type Snapshot struct {
	config   *params.PoseidonConfig // Consensus engine parameters to fine tune behavior
...
func loadSnapshot(config *params.PoseidonConfig, sigCache *lru.ARCCache, db ethdb.Database, hash common.Hash, ethAPI *ethapi.PublicBlockChainAPI) (*Snapshot, error) {
	blob, err := db.Get(append([]byte("poseidon-"), hash[:]...))
	if err != nil {
		return nil, err
	}
	snap := new(Snapshot)
...
func (s *Snapshot) store(db ethdb.Database) error {
	...
	return db.Put(append([]byte("poseidon-"), s.Hash[:]...), blob)
}
...
func (s *Snapshot) apply(headers []*types.Header, chain consensus.ChainHeaderReader, parents []*types.Header, chainId *big.Int) (*Snapshot, error) {
	...
	for _, header := range headers {
		...
		_, validator, err := ecrecover(header, s.sigCache, chainId)
		if err != nil {
			return nil, err
		}
