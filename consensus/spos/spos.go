...
// Spos is the consensus engine
type Spos struct {
	chainConfig *params.ChainConfig // Chain config
	config      *params.SposConfig  // Consensus engine configuration parameters for parlia consensus
...
func (p *Spos) IsSystemTransaction(tx *types.Transaction, header *types.Header) (bool, error) {
	// deploy a contract
	if tx.To() == nil {
		return false, nil
	}
...
func (p *Spos) IsSystemContract(to *common.Address) bool {
	if to == nil {
		return false
	}
...
func (p *Spos) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (p *Spos) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
...
func (p *Spos) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (p *Spos) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Spos) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Spos) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (p *Spos) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (p *Spos) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
...
// from.
func (p *Spos) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Spos) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	if !p.chainConfig.IsSpos(header.Number) {
		return fmt.Errorf("fork is not ready")
	}
...
// with.
func (p *Spos) Authorize(val common.Address, signFn SignerFn, signTxFn SignerTxFn) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *Spos) Delay(chain consensus.ChainReader, header *types.Header) *time.Duration {
	number := header.Number.Uint64()
	snap, err := p.snapshot(chain, number-1, header.ParentHash, nil)
	if err != nil {
		return nil
...
func (p *Spos) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
...
func (p *Spos) EnoughDistance(chain consensus.ChainReader, header *types.Header) bool {
...
// current signer.
func (p *Spos) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func (p *Spos) SealHash(header *types.Header) common.Hash {
...
func (p *Spos) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "parlia",
		Version:   "1.0",
...
func (p *Spos) getCurrentValidators(blockHash common.Hash) ([]common.Address, error) {
	// block
...
func (p *Spos) distributeIncoming(val common.Address, state *state.StateDB, header *types.Header, chain core.ChainContext,
...
// slash spoiled validators
func (p *Spos) slash(spoiledVal common.Address, state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
	// method
	method := "slash"
...
// init contract
func (p *Spos) initContract(state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
	// method
	method := "init"
	// contracts
...
func (p *Spos) distributeToSystem(amount *big.Int, state *state.StateDB, header *types.Header, chain core.ChainContext,
	txs *[]*types.Transaction, receipts *[]*types.Receipt, receivedTxs *[]*types.Transaction, usedGas *uint64, mining bool) error {
	// get system message
...
// slash spoiled validators
func (p *Spos) distributeToValidator(amount *big.Int, validator common.Address,
...
// get system message
func (p *Spos) getSystemMessage(from, toAddress common.Address, data []byte, value *big.Int) callmsg {
	return callmsg{
		ethereum.CallMsg{
			From:     from,
